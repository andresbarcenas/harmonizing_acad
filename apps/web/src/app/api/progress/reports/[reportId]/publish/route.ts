...
export async function POST(req: Request, { params }: { params: Promise<{ reportId: string }> }) {
  ...
  const updated = await db.$transaction(async (tx) => {
    ...
    const studentLocale = report.student.user.locale === "es" ? "es" : "en";
    ...
    if (shouldNotify) {
      await tx.notification.create({
        data: {
          ...
          title: studentLocale === "es" ? "Tu reporte de progreso está listo" : "Your progress report is ready",
          body: studentLocale === "es" ? "Ya puedes revisar el resumen mensual, calificación y próximos enfoques." : "You can now review the monthly summary, grade, and next focus areas.",
