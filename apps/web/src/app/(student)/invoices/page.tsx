  ...
  return (
    <AppShell role={viewer.role} activePath="/invoices" userName={viewer.name} locale={viewer.locale}>
      <PageIntro
        ...
        title={isSpanish ? "Tus facturas mensuales en un solo lugar." : "Your monthly invoices in one place."}
        description={isSpanish
          ? "Aquí verás las facturas emitidas por Harmonizing, sus PDFs y registros externos de Alegra cuando existan."
          : "Review Harmonizing invoices, private PDFs, and external Alegra records when available."}
      />

      <Card>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <CardTitle>{isSpanish ? "Facturas Harmonizing" : "Harmonizing invoices"}</CardTitle>
            <CardDescription>{isSpanish ? "Documentos internos de cobro emitidos por la academia." : "Internal billing documents issued by the academy."}</CardDescription>
          </div>
        ...
        <div className="mt-4 space-y-2">
          {data.nativeInvoices.map((invoice) => (
            <div key={invoice.id} className="rounded-[1.2rem] border border-[var(--color-border)] bg-white/68 px-4 py-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm font-semibold text-[var(--color-ink)]">{isSpanish ? "Factura" : "Invoice"} {invoice.invoiceNumber}</p>
          ...
          {!data.nativeInvoices.length ? (
            <p className="rounded-[1.1rem] border border-dashed border-[var(--color-border)] px-4 py-5 text-sm text-[var(--color-ink-soft)]">
              {isSpanish ? "Aún no hay facturas emitidas por Harmonizing." : "No Harmonizing invoices have been issued yet."}
            </p>
          ) : null}
        </div>
      </Card>
      ...
      <Card variant="subtle">
        ...
        <CardDescription>{isSpanish ? "Balance y uso de clases facturadas." : "Balance and usage of invoiced class credits."}</CardDescription>
      ...
      <Card variant="subtle">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <CardTitle>{isSpanish ? "Registros externos de Alegra" : "External Alegra records"}</CardTitle>
            <CardDescription>{data.isConfigured ? (isSpanish ? "Referencia histórica sincronizada desde Alegra." : "Historical reference synced from Alegra.") : (isSpanish ? "Alegra aún no está configurado." : "Alegra is not configured yet.")}</CardDescription>
          </div>
          {data.isConfigured ? <StudentInvoiceSyncButton locale={viewer.locale} /> : null}
        </div>

        {data.isConfigured ? (
          <>
            {data.isStale ? (
              <div className="mt-3 rounded-[1rem] border border-[var(--color-border)] bg-white/68 px-3 py-2 text-xs text-[var(--color-ink-soft)]">
                {data.lastSyncedAt
                  ? `${isSpanish ? "Última sincronización" : "Last sync"}: ${new Date(data.lastSyncedAt).toLocaleString(intlLocale(viewer.locale))}`
                  : isSpanish ? "Aún no se ha completado la primera sincronización." : "The first sync has not completed yet."}
                {data.link?.lastError ? <p className="mt-1 text-rose-700">{data.link.lastError}</p> : null}
              </div>
            ) : null}
            <div className="mt-4 space-y-2">
              {data.invoices.map((invoice) => (
                <div key={invoice.id} className="rounded-[1.2rem] border border-[var(--color-border)] bg-white/68 px-4 py-3">
                  ...
                  <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                    ...
                    {invoice.pdfUrl ? <a href={invoice.pdfUrl} target="_blank" rel="noreferrer" className="w-full sm:w-auto"><Button size="sm" variant="outline" className="w-full sm:w-auto">{isSpanish ? "PDF Alegra" : "Alegra PDF"}</Button></a> : null}
                  </div>
                </div>
              ))}
