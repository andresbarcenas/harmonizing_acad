    ...
    <AppShell role={viewer.role} activePath="/invoices" userName={viewer.name} locale={viewer.locale}>
      <PageIntro
        ...
        description={data.isConfigured
          ? viewer.locale === "es" ? "Sincronizamos tus comprobantes desde Alegra para que puedas ver estado, montos y descargar PDF cuando esté disponible." : "We sync your invoices from Alegra so you can review status, amounts, and download PDFs when available."
          : viewer.locale === "es" ? "Esta sección estará disponible cuando la integración de facturación esté activa." : "This section will be available when invoice integration is active."}
      >
      ...
      {!data.isConfigured ? (
        <Card>
          <CardTitle>{viewer.locale === "es" ? "Las facturas aún no están configuradas." : "Invoices are not configured yet."}</CardTitle>
          <CardDescription>
            {viewer.locale === "es"
              ? "El equipo de Harmonizing activará esta sección cuando la integración de facturación esté lista."
              : "The Harmonizing team will enable this section when invoice integration is ready."}
          </CardDescription>
        </Card>
      ) : (
        <>
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <CardDescription>{viewer.locale === "es" ? "Total sincronizado" : "Synced total"}: {data.totalInvoices}</CardDescription>
              </div>
