              ...
              {item.attachments.length ? (
                <div className="mt-3 flex flex-wrap gap-2">
                  {item.attachments.map((attachment) => (
                    ...
                    >
                      {isSpanish ? "Partitura" : "Sheet"}: {attachment.originalName}
                    </a>
                  ))}
                </div>
              ) : null}
    ...
    <Card>
      ...
      {!report ? <EmptyState text={isSpanish ? "Aún no hay reportes generados." : "No reports yet."} /> : (
        <div className="mt-3 rounded-xl border border-[var(--color-border)] bg-white/72 p-3 text-sm">
          ...
          <p className="mt-1 text-xs text-[var(--color-ink-soft)]">{report.studentVisibleSummary ?? report.teacherSummary ?? (isSpanish ? "Reporte publicado por la academia." : "Report published by the academy.")}</p>
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-[var(--color-ink-soft)]">
            ...
            <span>{isSpanish ? "Calificación" : "Grade"}: {report.gradeLetter ?? report.finalGrade ?? "-"}</span>
          </div>
          <Link href={`/progress/reports/${report.id}`} className="mt-3 inline-flex text-xs font-semibold text-[var(--color-gold-deep)]">
            {isSpanish ? "Abrir reporte completo" : "Open full report"}
          </Link>
        </div>
      )}
    </Card>
