...
function logoHtml(locale: AppLocale) {
  const subtitle = locale === "es" ? "Academia musical" : "Music academy";
  return `
    <div style="display:inline-flex;align-items:center;gap:12px;margin-bottom:22px;">
      ...
      <div>
        ...
        <div style="font-size:9px;letter-spacing:.34em;text-transform:uppercase;color:#9a8f82;margin-top:6px;">${escapeHtml(subtitle)}</div>
      </div>
    </div>`;
}
...
function welcomeBullets(input: WelcomeEmailInput) {
  ...
  if (input.role === Role.TEACHER) {
    return isSpanish
      ? [
          ...
          "Da comentarios sobre videos de práctica y acompaña el progreso de tus estudiantes.",
        ]
      : [
  ...
  if (input.role === Role.PARENT) {
    return isSpanish
      ? [
          input.teacherName ? `La docente asignada aparecerá como ${input.teacherName}.` : "La docente asignada aparecerá en el portal familiar.",
          "Desde tu cuenta familiar podrás ver agenda, progreso, videos, mensajes, consentimientos y facturas del estudiante vinculado.",
          "También podrás firmar consentimientos y solicitar cambios de horario en nombre del estudiante.",
          "Los montos de factura se sincronizan desde Alegra cuando estén disponibles.",
        ]
      : [
          input.teacherName ? `The assigned teacher will appear as ${input.teacherName}.` : "The assigned teacher will appear in the family portal.",
          "From your family account you can view schedule, progress, videos, messages, consents, and invoices for the linked student.",
          "You can also sign consent forms and request schedule changes on behalf of the student.",
          "Invoice amounts sync from Alegra when available.",
        ];
  }
  ...
  return isSpanish
    ? [
        ...
        input.planLabel ? `Tu cantidad de clases registrada es ${input.planLabel}.` : "Tus clases disponibles aparecerán en tu panel.",
    ...
    : [
        ...
        input.planLabel ? `Your recorded class allowance is ${input.planLabel}.` : "Your available classes will appear in your dashboard.",
...
function welcomeEmailHtml(input: WelcomeEmailInput) {
  ...
  const roleLabel = input.role === Role.TEACHER
    ...
    : input.role === Role.PARENT
      ? isSpanish ? "acudiente/familia" : "parent/guardian"
